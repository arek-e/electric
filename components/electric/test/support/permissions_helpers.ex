defmodule ElectricTest.PermissionsHelpers do
  ...
  defmodule Perms do
    ...
    def rules({MockSchemaLoader, state}, %P.Rules{} = rules) do
      {:ok, state} = MockSchemaLoader.save_global_permissions(state, rules)

      {{MockSchemaLoader, state}, rules}
    end

    def rules({MockSchemaLoader, state}, attrs) do
      {:ok, old_rules} = MockSchemaLoader.global_permissions(state)

      parent_id = Keyword.get(attrs, :parent_id, old_rules.id)
      id = Keyword.get(attrs, :id, parent_id + 1)
      ...
      rules({MockSchemaLoader, state}, rules)
    end
